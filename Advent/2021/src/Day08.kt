...
fun output(input: List<String>) = input.map { it.substringAfter('|').trim().split(' ') }
...
fun main() {
  fun part1(input: List<String>) = output(input).flatten().count { it.length in listOf(2, 3, 4, 7) }

  fun part2(input: List<String>) = input.map { line -> line.split(" | ").map { it.split(" ") } }
    .map { (left, right) ->
